...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  error_class_to_hid = (model.hid_to_class' * error) .* (logistic(hid_input) .* (1 - logistic(hid_input)));

  ret.hid_to_class = (error * hid_output') / m + wd_coefficient .* model.hid_to_class;
  ret.input_to_hid = (error_class_to_hid * data.inputs') / m + wd_coefficient .* model.input_to_hid;
end
